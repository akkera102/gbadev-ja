...
//---------------------------------------------------------------------------
#define PATTERN_LENGTH 64	// Maximum pattern length
...
typedef struct {
	u16 channel_period;
	u16 toneporta_target;
	u8  channel_note;
	u8  highmask;
	u8  vibrato_tremolo_phase;
	u8  envelope;
	u16 table;
	u8  table_row;
} UGE_ST_CHANNEL1;
...
EWRAM_CODE void UgeInit(ST_UGE_SONG* s)
{
	UgeSong.tempo     = s->tempo;
	UgeSong.order_cnt = s->order_cnt;
	UgeSong.order1    = s->order1;
	UgeSong.order2    = s->order2;
	UgeSong.order3    = s->order3;
	UgeSong.order4    = s->order4;
	UgeSong.duty      = s->duty;
	UgeSong.wave      = s->wave;
	UgeSong.noise     = s->noise;
	UgeSong.routines  = s->routines;
	UgeSong.waves     = s->waves;

	UgeCh1.envelope = 0xf0;
	UgeCh2.envelope = 0xf0;

	#define hUGE_NO_WAVE 100
	current_wave = hUGE_NO_WAVE;

	UgeLoadPatterns();
}
//---------------------------------------------------------------------------
// Sets all 4 pattern pointers from a certain index in the respective 4 orders.
EWRAM_CODE void UgeLoadPatterns(void)
{
	pattern1 = *UgeSong.order1;
	pattern2 = *UgeSong.order2;
	pattern3 = *UgeSong.order3;
	pattern4 = *UgeSong.order4;
}
//---------------------------------------------------------------------------
//; Sets a channel's muting status.
EWRAM_CODE void UgeSetMute(enum UGE_ID_CHANNEL ch, enum UGE_ID_MUTE mute)
{
/*
...
play_ch3_note:
    ...
    // To avoid this, we kill the wave channel (0 Å® NR30), then re-enable it.
...
//; Destroy: AF BC DE HL
hUGE_dosound::
EWRAM_CODE void UgeDoSound(void)
{
